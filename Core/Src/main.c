...
#define DBG_RING_SZ 256
volatile uint8_t  dbg_ring[DBG_RING_SZ];
volatile uint16_t dbg_head = 0;        /* watch these two in Live     */
uint8_t           dbg_rx_byte;         /* Expressions / Live Watch
/* USER CODE END PV */
...
volatile uint32_t g_tach_pulses = 0;
/* USER CODE END 0 */
...
int main(void)
{
  ...
  ssd1306_init();     /* Initialise OLED driver                          */
  ThermalApp_Init();  /* Start PWM, zero buffers, display splash screen  */
  /* USER CODE END 2 */
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    if (huart->Instance == USART1) {
        dbg_ring[dbg_head % DBG_RING_SZ] = dbg_rx_byte;
        dbg_head++;
        HAL_UART_Receive_IT(&huart1, &dbg_rx_byte, 1);
    }
}
...
void Error_Handler(void)
{
  ...
  while (1)
  {
    /* Spin here; attach a debugger and inspect the call stack. */
  }
...
{
  /* USER CODE BEGIN 6 */
  /* Example: printf("Wrong parameter: file %s, line %d\r\n", file, line); */
  (void)file;
  (void)line;
  /* USER CODE END 6 */
}
